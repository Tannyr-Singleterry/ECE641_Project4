...
output            READY ,
output            SCL , 
//inout             SDA , // DG commented
input			  SDAI, // DG added
output			  SDAO,  // DG added
output [9:0]      STATUS 
...
assign STATUS[9:0]  = { VCM_END , STEP[6:0] ,SCL,SDAI  } ;
...
VCM_I2C i2c2( 
   ...
   .CLK_50     ( CLK_50 ),
   .I2C_SCL    ( SCL    ) ,
   .I2C_SDAI    ( SDAI    ),  // DG Modified from SDA
   .I2C_SDAO   (  SDAO ),  // Added by DG
   .VCM_DATA   ( VCM_DATA  ), 
...
always @( posedge VIDEO_CLK ) 
   {oR, oG, oB}  <=   ((SW_FUC_LINE &  LINE )  ||  ( SW_FUC_ALL_CEN  &  ( ~VCM_END  &  LINE) ) ) ?  
					{8'hFF, 8'hFF, 8'h0} : 
						( !SW_Y )?  //( ACTIV_C ? 24'h555555 :  
							{ iR, iG, iB }   : 
								(SW_H_FREQ)?  {Y[15:8 ], Y[15:8 ], Y[15:8 ] } :  {S[7:0], S[7:0],  S[7:0] };
