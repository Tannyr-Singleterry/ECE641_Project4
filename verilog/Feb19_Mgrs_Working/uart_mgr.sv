module uart_mgr(ar, clk, new_rx, tx_done, tx_start, rx_word, tx_word, control_regs,
	 mem_wr_req, mem_rd_req, mem_addr, mem_wr_data, mem_rd_data, mem_rd_valid, mem_busy,
	 wr_next);
	...
	parameter BL = 8;	// Burst length for SDRAM reads and writes
	...
	if(~ar)
	   ...
	   case(cs)
			...
			Wait_Mem:
				...
				if(mem_rd_valid)
					begin
					// Need to adapt for BL > 1
					tx_reg = mem_rd_data;  // Continue reading same data for burst
					ctr = ctr + 1;
					end
			...
			Start_WriteDataMem:
				if(~mem_busy)
					...
					mem_wr_req = 1'b1;
					ctr = 0;
					cs = Continue_WriteMem;
					end
				
			Continue_WriteMem:
				if(ctr < BL+5)  // feb12 +5 (5 is based in ST testing)
					begin
					mem_wr_req = 1'b0;
					ctr = ctr + 1;
					
					//if(ctr > 6)
					if(wr_next)  // provide new write data
						mem_wr_data = mem_wr_data + 1;  // Continue writing same data for burst
					end
				else
					begin
					mem_wr_req = 1'b0;
