module sdram_mgr(
	...
	data_fifo wr_data_fifo (
		...
		.clock(clk),        				//            .clk,          Positive-edge-triggered clock.
		...
		.usedw(wr_data_fifo_usedw),        //            .usedw,        Show the number of words stored in the FIFO.
		.full(wr_data_fifo_full),         //            .full,         When full signal is asserted, the FIFO IP core is considered full. Do not perform write request operation when the FIFO IP core is full.
		.empty(wr_data_fifo_empty),        //            .empty,        When empty signal is asserted, the FIFO IP core is considered empty.Do not perform read request operation when the FIFO IP core is empty.
	...
	data_fifo rd_data_fifo (
		...
		.usedw(rd_data_fifo_usedw),        //            .usedw,        Show the number of words stored in the FIFO.
		.full(rd_data_fifo_full),         //            .full,         When full signal is asserted, the FIFO IP core is considered full. Do not perform write request operation when the FIFO IP core is full.
		.empty(rd_data_fifo_empty),        //            .empty,        When empty signal is asserted, the FIFO IP core is considered empty.Do not perform read request operation when the FIFO IP core is empty.
...
address_fifo rd_addr_fifo (
	...
	// Start read when controller idle, rd adddress availabble, and at least 8 locations available in FIFO
	assign read_start = ~mem_busy & ~rd_addr_fifo_empty & ~rd_data_fifo_almost_full;

	// Start write when controller idle, wr adddress availabble, and at least 8 words available in FIFO
	assign write_start = ~mem_busy & ~wr_addr_fifo_empty & ~wr_data_fifo_almost_empty;
